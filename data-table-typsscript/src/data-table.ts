export class DataTable extends HTMLElement {
    ...
    renderBody() {
      let result = '';
      this.data.filter((row: any, index: number) => {
    ...
    renderHeader() {
      let header = '<tr>';
      this.cols!.map(col => {
        header += `<th data-sort="${col}">${col} <img data-sort="${col}" src="${this.sorticon}" style="height:16px;width:16px"/></th>`;
      });
    ...
    static get observedAttributes() { return ['src', 'cols', 'sorticon']; }
